...
/**
 * Primary Procbased capabilities of the vm
 * Refer from SDM volume 3
*/
struct capability_info vm_primary_procbased[22] =
{
...
/**
 * Secondary Procbased capabilities of the vm
 * Refer from SDM volume 3
*/
struct capability_info vm_secondary_procbased[27] =
{
...
/**
 * Tertiary Procbased capabilities of the vm
 * Refer from SDM volume 3
*/
struct capability_info vm_tertiary_procbased[4] =
{
        { 0, "LOADIWKEY Exiting" },
    { 1, "Enable HLAT" },
...
detect_vmx_features(void)
{
        ...
        /* Primary Procbased controls of the vm */
        rdmsr(IA32_VMX_PROCBASED_CTLS, lo, hi);
        pr_info("Procbased Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vm_primary_procbased, 22, lo, hi);


        /* Secondary Procbased controls of the vm */
        rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
        pr_info("Procbased Secondary Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vm_secondary_procbased, 27, lo, hi);

        /* Tertiary Procbased controls of the vm */
        rdmsr(IA32_VMX_PROCBASED_CTLS3, lo, hi);
        pr_info("Procbased tertiary Controls MSR: 0x%llx\n",
                (uint64_t)(lo | (uint64_t)hi << 32));
        report_capability(vm_tertiary_procbased, 4, lo, hi);

        /* Exit control capabilities of the vm */
        ...
        /* Entry control capabilities of the vm */
